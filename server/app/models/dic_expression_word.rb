...
class DicExpressionWord < CategorisedWord
  ...
  def self.generate_record!
    ...
    url_docs = DicExpressionWord.request_and_parse_html(url: host_url.to_s)
    urls = url_docs.css(".mainWL").css("a").map{ |anchor| anchor[:href] }
    urls.each do |url|
      (1...1000).each do |i|
        word_docs = DicExpressionWord.request_and_parse_html(url: url.to_s + "/" + i.to_s)
        docs = word_docs.css(".crosslink").map{|d| {d[:href] => d.text} }
        break if docs.blank?
        docs.each do |doc|
          doc.each do |url, text|
            dc = DicExpressionWord.request_and_parse_html(url: url)
