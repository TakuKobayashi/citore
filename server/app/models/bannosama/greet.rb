...
class Bannosama::Greet < ApplicationRecord
  ...
  def generate_thumnail!(file)
    ...
    image.resize(Datapool::ImageMetum.calc_resize_text(width: image.width, height: image.height, max_length: 200))
  ...
  def upload_s3_and_set_audiofile(audio_file)
    ...
    self.audio_upload_url = Datapool::ResourceMetum::S3_ROOT_URL + filepath
  ...
  def get_thumbnail_url
    ...
    return Datapool::ResourceMetum::S3_ROOT_URL + Bannosama::GreetImage::IMAGE_S3_THUMBNAIL_ROOT + self.id.to_s + File.extname(image.upload_url).downcase
  end
end
