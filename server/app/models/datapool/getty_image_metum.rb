...
class Datapool::GettyImageMetum < Datapool::ImageMetum
  ...
  def self.crawl_images!(keyword:)
    ...
    loop do
      images = []
      json = RequestParser.request_and_parse_json(url: GETTY_IMAGES_API_URL, params: {phrase: keyword, page_size: 100, page: page, fields: "detail_set"}, header: {"Api-Key" => ENV.fetch('GETTY_IMAGES_STANDARD_KEY', '')})
